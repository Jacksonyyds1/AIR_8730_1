...
PositionTracker::PositionTracker(EncoderMap &encoder_map, const uint8_t *pattern, uint16_t pattern_length)
...
ValidationResult PositionTracker::track_position(uint8_t bit, SearchDirection direction, uint16_t &tracked_position)
{
    ...
    // 计算下一个预期位置和事件
    uint16_t expected_position = PositionCalculator::next_position(current_position_, direction, pattern_length_);
...
ValidationResult PositionTracker::handle_tracking_event(TrackerEvent event, uint8_t bit, SearchDirection direction, uint16_t &position)
{
    ...
    case TrackerEvent::MATCH:
        current_position_ = PositionCalculator::next_position(current_position_, direction, pattern_length_);
...
uint16_t PositionTracker::PositionCalculator::next_position(uint16_t current, SearchDirection direction, uint16_t pattern_length)
{
...
int PositionTracker::PositionCalculator::calculate_distance(uint16_t pos1, uint16_t pos2, uint16_t pattern_length)
{
