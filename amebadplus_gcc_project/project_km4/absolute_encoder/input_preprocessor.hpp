...
public:
    ...
    struct ProcessingResult
    {
        ...
        float prediction_accuracy;          // 预测精度 (0.0-1.0)
        ...
        ProcessingResult() : should_sample_signal(false), unit_completed(false),
            ...
            prediction_accuracy(0.0f), new_state(ProcessingState::INITIALIZING) {}
    };
...
public:
     ...
     * 获取预测精度统计
     */
    float get_average_prediction_accuracy() const;
...
private:
    ...
    int32_t steps_since_last_edge_;         // 距离上次边沿的步数
    ...
    // 统计和校准
    std::deque<float> prediction_accuracy_history_;  // 预测精度历史
    uint32_t total_edges_detected_;         // 总检测边沿数
    uint32_t successful_predictions_;       // 成功预测数
    uint32_t direction_changes_;            // 方向变化次数
     ...
     */
    void update_prediction_statistics(int32_t predicted_pos, int32_t actual_pos);
     ...
     * 自适应调整预测参数
     */
    void adaptive_adjust_prediction();
     ...
     * @return 处理结果
     */
    ProcessingResult handle_edge_synchronization(uint8_t sensor_signal);
};
