...
public:
     ...
     */
    PositionTracker(EncoderMap &encoder_map, const uint8_t *pattern, uint16_t pattern_length);
...
private:
    ...
    {
        static uint16_t next_position(uint16_t current, SearchDirection direction, uint16_t pattern_length);
        static int calculate_distance(uint16_t pos1, uint16_t pos2, uint16_t pattern_length);
    };
};
