...
typedef struct {
    ...
    volatile bool isr_lock;
    bool is_first_edge;
    uint32_t last_pulse_interval;
} fan_fg_runtime_t;
...
int32_t pid_iterate(pid_control_block_t *pcb, int32_t target_value, int32_t feedback_value)
{
    ...
    int32_t err_delta = error - pcb->last_error;
    pcb->err_integral += error;
...
void fan_speed_control_task(void *param)
{
    ...
    while(1) {
        ...
        if(pulse_cnt > 0) {
            total_time_us = fg.last_pulse_interval * pulse_cnt;
        }
