...
InputPreprocessor::ProcessingResult InputPreprocessor::handle_initialization(uint8_t sensor_signal)
{
    ...
    bool threshold_reached = steps_since_last_unit_ >= static_cast<int32_t>(config_.steps_per_encoder_unit * 0.1f);
...
InputPreprocessor::ProcessingResult InputPreprocessor::handle_tracking(uint8_t sensor_signal)
{
    ...
    // 计算在当前编码器单位区间内的位置
    int32_t steps_from_last_unit = steps_since_last_unit_;
    ...
    if(edge_detected || unit_complete || timeout)
    {
        ...
        // 更新统计信息
        steps_since_last_unit_ = 0;       // 重置区间计数，开始新的编码器单位
...
int32_t InputPreprocessor::calculate_based_edge_for_sample(StepDirection from_direction, StepDirection to_direction)
{
    ...
    if(from_direction == to_direction)
    {
        ...
        return current_reference_.step_position + std::copysignf(config_.steps_per_encoder_unit, direction_multiplier);
    }
    else
    {
        // 方向改变，使用当前进度计算预测位置
        ...
        // 新方向的第一个边沿应该在剩余距离处
        float direction_multiplier = (to_direction == StepDirection::FORWARD) ? 1.0f : -1.0f;
        return current_step_position_ + std::copysignf(config_.steps_per_encoder_unit - steps_since_last_unit_, direction_multiplier);
    }
}
...
bool InputPreprocessor::handle_direction_change(StepDirection new_direction)
{
    ...
    // 预测新方向使用的基准边沿位置
    steps_since_last_unit_ = calculate_based_edge_for_sample(current_direction_, new_direction);
...
float InputPreprocessor::get_estimated_unit_offset()
{
    // 根据当前步数和采样点比例估算偏移位置
    if(current_direction_ == StepDirection::FORWARD)
    {
        return static_cast<float>(steps_since_last_unit_) / config_.steps_per_encoder_unit;
    }
    else
    {
        // 反向时，偏移位置需要反转
        return 1.0f - static_cast<float>(steps_since_last_unit_) / config_.steps_per_encoder_unit;
    }
}
...
bool InputPreprocessor::should_trigger_edge_sampling()
{
    ...
    int32_t steps_from_last_edge = steps_since_last_unit_;
